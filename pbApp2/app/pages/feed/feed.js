            ...
            for (var i = 0; i < list.length; i++) {
                ...
                if (list[i].MediaThumb != undefined) {
                    for (var t = 0; t < list[i].MediaThumb.length; t++) {
                        ...
                        var imgUrl = 'http://api1.myplaybook.co.nz/api' + "/PostMedia" + '?api_key=' + encodeURIComponent(token) + "&path=" + encodeURIComponent(list[i].MediaThumb[t].Path) + "&filename=" + encodeURIComponent(list[i].MediaThumb[t].Filename);
